...
import dev_Tools.demonek.utils.StringUtil;
...
public class ExcGameMode implements Executor{
	...
	public void execute(CommandSender sender, String[] args) {
		...
		if(!sender.hasPermission("dev_Tools.cmd.gamemode")){
			Util.sendMessage(sender, "&cNie masz uprawnien do uzycia tej komendy &8(&7dev_Tools.cmd.gamemode&8)");
			return;
		}
		...
		if(args.length == 0){
			...
			if(args.length == 1){
				...
				if(args.length == 2){
					...
					if(!p.hasPermission("dev_Tools.cmd.gamemode.other")){
						Util.sendMessage(p, "&cNie masz uprawnien do uzycia tej komendy &8(&7dev_Tools.cmd.gamemode.other&8)");
						return;
					}
